...
class SAR(nn.Module):
    ...
    def __init__(
        ...
        self.num_forwards = 0
        self.num_backwards = 0  # number of samples after Second filtering, exclude both unreliable and redundant samples
        self.steps = steps
        assert steps > 0, "SAR requires >= 1 step(s) to forward and update"
        self.episodic = episodic

        self.margin_e0 = (
            margin_e0  # margin E_0 for reliable entropy minimization, Eqn. (2)
        )
        self.reset_constant_em = (
            reset_constant_em  # threshold e_m for model recovery scheme
        )
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, ema, reset_flag, num_forward, num_backward = forward_and_adapt_sar(
                x,
                self.model,
                self.optimizer,
                self.margin_e0,
            ...
            self.num_forwards += num_forward
            self.num_backwards += num_backward
            if wandb.run is not None and wandb.run.summary is not None:
                wandb.run.summary["num_forwards"] = self.num_forwards
                wandb.run.summary["num_backwards"] = self.num_backwards
            if reset_flag:
                self.reset()
...
def forward_and_adapt_sar(x, model, optimizer, margin, reset_constant, ema):
    ...
    """
    num_forward, num_backward = 0, 0
    optimizer.zero_grad()
    # forward
    outputs = model(x)
    num_forward += x.shape[0]
    # adapt
    ...
    if filter_ids_1[0].size(0) > 0:
        num_forward += x.shape[0]
        outputs = model(x)
        entropys2 = softmax_entropy(outputs)
        ...
        num_backward += entropys2[filter_ids_2].numel()
    ...
    return outputs, ema, reset_flag, num_forward, num_backward
...
def check_model(model):
    ...
    assert has_any_params, "SAR needs params to update: check which require grad"
    assert not has_all_params, (
        "SAR should not update all params: check which require grad"
    )
    has_norm = any(
        [
            isinstance(m, (nn.BatchNorm2d, nn.LayerNorm, nn.GroupNorm))
            for m in model.modules()
        ]
    )
