"""
Source: https://github.com/mr-eggplant/FOA/blob/main/tta_library/cotta.py
...
class CoTTA(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.steps):
            outputs, num_forwards = self.forward_and_adapt(
        ...
        if wandb.run is not None and wandb.run.summary is not None:
            wandb.run.summary["num_forward"] = self.num_forwards
            wandb.run.summary["num_backward"] = self.num_backwards
        return outputs
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        if to_aug:
            ...
            num_forwards = outputs_ema.size(0) * outputs_ema.size(1)
            outputs_ema = outputs_ema.mean(0)
        else:
            outputs_ema = standard_ema
            num_forwards = outputs_ema.size(0)
        num_forwards += outputs.size(0)
        # Augmentation-averaged Prediction
        # Student update
        ...
        return outputs_ema, num_forwards
