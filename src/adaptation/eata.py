...
class EATA(nn.Module):
    ...
    def forward(self, x):
        ...
        if wandb.run is not None and wandb.run.summary is not None:
            wandb.run.summary["num_samples_update_2"] = self.num_samples_update_2
            wandb.run.summary["num_samples_update_1"] = self.num_samples_update_1
...
def forward_and_adapt_eata(
    ...
    # forward
    outputs = model(x)
    # NOTE: since we remove ImageNetMask related code, so d_margin is not adjusted here.
    # Instead, it is adjusted through config directly.
    # adapt
    ...
    if current_model_probs is not None:
        ...
        updated_probs = update_model_probs(
            current_model_probs, outputs[filter_ids_1][filter_ids_2].softmax(1).detach()
        )
    else:
        updated_probs = update_model_probs(
            current_model_probs, outputs[filter_ids_1].softmax(1).detach()
        )
    ...
    if fishers is not None:
        ...
        for name, param in model.named_parameters():
            if name in fishers:
                ewc_loss += (
                    fisher_alpha
                    * (fishers[name][0] * (param - fishers[name][1]) ** 2).sum()
                )
        loss += ewc_loss
    if x[ids1][ids2].size(0) != 0:
        loss.backward()
        optimizer.step()
    optimizer.zero_grad()
    c1, c2 = entropys.size(0), filter_ids_1[0].size(0)
    del loss, entropys, filter_ids_1, ids1, ids2, coeff
    torch.cuda.empty_cache()

    return outputs.detach(), c1, c2, updated_probs
...
def update_model_probs(current_model_probs, new_probs):
    if current_model_probs is None:
        ...
        if new_probs.size(0) == 0:
            ...
            with torch.no_grad():
                return (
                    0.9 * current_model_probs + (1 - 0.9) * new_probs.mean(0).detach()
                )
