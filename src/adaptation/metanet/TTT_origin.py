...
class TTTBase(nn.Module):
    ...
    def _init_qkvo_proj(self):
        ...
        self.o_proj = nn.Linear(
            self.width, self.num_heads * self.head_dim // 2, bias=False
        )
...
class TTTMetanet(nn.Module):
    ...
    def forward(
        ...
        # 新加，对参数降维使其能匹配实际梯度的维度
        # hidden_states = self.grad_proj(hidden_states)

        # 对保存的新参数进行detach，截断历史计算图
        for name in last_mini_batch_params_dict.keys():
            last_mini_batch_params_dict[name] = last_mini_batch_params_dict[
                name
            ].detach()

        return hidden_states, last_mini_batch_params_dict
...
if __name__ == "__main__":
    ...
    ttt_metanet = TTTMetanet(configuration, 0)
    ttt_metanet.eval()
    for name, param in ttt_metanet.named_parameters():
    ...
    # 计算参数量
    total = 0
    for p in ttt_metanet.parameters():
        total += p.numel()
    print(f"Total number of parameters: {total}")
    optimizer = torch.optim.SGD(ttt_metanet.parameters(), lr=0.01)
    last_mini_batch_params_dict = None

    start = time.time()
    for _ in range(50):
        inputs = torch.rand(100, 1, 768)  # bs, seq_len, hidden_sizes
        outputs, last_mini_batch_params_dict = ttt_metanet(
    ...
    start = time.time()
    for i in range(50):
        inputs = torch.rand(10, 1, 768)  # bs, seq_len, hidden_sizes
        outputs = []
        for i in range(10):
            inputs_i = inputs[i : i + 1]
            outputs_i, last_mini_batch_params_dict = ttt_metanet(
                inputs_i, last_mini_batch_params_dict=None
            )
