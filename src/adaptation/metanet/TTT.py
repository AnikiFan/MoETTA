"""
We directly modified the dimensions of qkv in TTT layer to align with the input and output dimensions.
"""
...
def scan(f, init, xs, out, checkpoint_group=0):
    ...
    def scan_fn(carry, i_start, i_end):
        for i in range(
            i_start, i_end
        ):  # 循环minibatch，串行输入得到新的W，再把新的W用于下一个输入
            if isinstance(xs, dict):
...
class TTTBase(nn.Module):
    def __init__(
        ...
        self.mini_batch_size = config.mini_batch_size
        self.input_dim = input_dim if input_dim is not None else self.width
        self.output_dim = output_dim if output_dim is not None else self.width
    ...
    def _init_qkvo_proj(self):
        ...
        self.o_proj = nn.Linear(self.width, self.output_dim, bias=False)
    ...
    def _init_ttt_lr_gate(self):
        # [width, 1]
        linear_weight_data = nn.Linear(self.input_dim, 1, bias=True).weight.data
        ...
        linear_bias_data = nn.Linear(self.input_dim, 1, bias=True).bias.data
    ...
    def get_ttt_inputs(self, inputs, mini_batch_size, cache_params):
        ...
        X = X.reshape(B, num_mini_batch, mini_batch_size, self.input_dim)
    ...
    def forward(
        ...
        cache_params: Optional[TTTCache] = None,
        last_mini_batch_params_dict: Optional[dict] = None,  # 新加
    ):
        B, L = hidden_states.shape[:2]
        reminder_len = L % self.mini_batch_size
        num_mini_batch = L // self.mini_batch_size
        # last_mini_batch_params_dict = None #默认为None
...
class TTTLinear(TTTBase):
    def __init__(
        ...
        super().__init__(config, layer_idx, input_dim, output_dim)
    ...
    def ttt(
        ...
        if last_mini_batch_params_dict is not None:
            ...
            init_params_dict = {
                ...
                ),  # 根据bs复制tensor，使其维度能对上，相当于expand？
...
class TTTMGG(nn.Module):
    def __init__(
        self, config: TTTConfig, layer_idx: int, input_dim: int, output_dim: int
    ):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.seq_modeling_block = ttt_layer(
            config=config,
            layer_idx=layer_idx,
            input_dim=input_dim,
        ...
        # self.grad_proj = nn.Linear(config.hidden_size, config.hidden_size // 2)
    ...
    def forward(
        ...
        # TTT Layer
        hidden_states, last_mini_batch_params_dict = self.seq_modeling_block(
            hidden_states=hidden_states,
            attention_mask=attention_mask,
            position_ids=position_ids,
            cache_params=cache_params,
            last_mini_batch_params_dict=last_mini_batch_params_dict,  # 新加
        )

        # Truncat computation graph
        for name in last_mini_batch_params_dict.keys():
            last_mini_batch_params_dict[name] = last_mini_batch_params_dict[
                name
            ].detach()

        return torch.tanh(
            hidden_states
        ), last_mini_batch_params_dict  # Add activation function tanh


TTT_STANDARD_CONFIGS = {
    "125m": {
        "hidden_size": 768,
        "intermediate_size": 2048,  # 用于残差块的mlp
...
if __name__ == "__main__":
    ...
    # configuration.mini_batch_size = 1
    ttt_mgg = TTTMGG(configuration, 0, input_dim=2, output_dim=1)
    ttt_mgg.eval()
    for name, param in ttt_mgg.named_parameters():
        print(name, param.shape)

    print(ttt_mgg)

    total = 0
    for p in ttt_mgg.parameters():
        total += p.numel()
    print(f"Total number of parameters: {total}")
    optimizer = torch.optim.SGD(ttt_mgg.parameters(), lr=0.01)
    last_mini_batch_params_dict = None

    start = time.time()
    for _ in range(50):
        inputs = torch.rand(4, 1, 2)  # bs, seq_len, hidden_sizes
        outputs, last_mini_batch_params_dict = ttt_mgg(
            inputs, last_mini_batch_params_dict=None
        )
        optimizer.zero_grad()
